...
image_folder_path = f'D:\\A_Project\\PyCharm_Project\\CameraCalibration\\Data\\Image\\Data_({index_folder})'
image_files = [f for f in os.listdir(image_folder_path) if f.lower().endswith('.jpg')]

position_folder_path = f'D:\\A_Project\\PyCharm_Project\\CameraCalibration\\Data\\Position\\Data_({index_folder})'
...
result_folder_path = f'D:\\A_Project\\PyCharm_Project\\CameraCalibration\\Target2cam\\Data_({index_folder})'
value_folder_path = f'D:\\A_Project\\PyCharm_Project\\CameraCalibration\\Value\\Target2cam\\Data_({index_folder})'

for index in range(1, len(image_files) + 1):
    image = f'D:\\A_Project\\PyCharm_Project\\CameraCalibration\\Data\\Image\\Data_({index_folder})\\image_({index}).jpg'
    txt = f'D:\\A_Project\\PyCharm_Project\\CameraCalibration\\Data\\Position\\Data_({index_folder})\\position_({index}).txt'
